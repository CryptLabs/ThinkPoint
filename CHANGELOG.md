...
- About screen (`i`): logo, version, author, website, source and licence, plus
  what the current session found — whether xinput is available, how many
  devices were listed and whether sudo will prompt. The version also appears in
  the title bar.
- Detached (floating) devices are listed and `t` reattaches them to the master
  pointer and enables them in one step.
...
- Release workflow: tagging builds x86_64 and aarch64 tarballs and a Debian
  package, publishes them with checksums, and takes the release notes from this
  file.

### Changed

- The README's drift section is a walkthrough now: measure first, what the
  preset changes and why, how to tell whether it worked, and the physical
  causes left over when it cannot help.
- The PKGBUILD lives in `packaging/aur/`, declares `gcc-libs` and `glibc`,
  which the binary links against, and lists `sudo` under `optdepends` beside
  `xinput` — neither is needed to run the tool, only for the parts that use
  them.
...
- A detached device no longer disappears from the list. `xinput list` labels a
  ...
  filtered out along with the keyboards and there was no way to recover it from
  the interface.
- Saving the udev rule no longer drops settings persisted in an earlier
  ...
  setting would have left it out. The existing rule file is read and merged.
- Saving the X profile has the same fix, and settings for devices that are not
  connected today are kept rather than pruned.
- The PKGBUILD unpacked into a directory that does not exist. GitHub names its
  archive after the repository, which is capitalised, so the tarball extracts to
  `ThinkPoint-0.3.0` rather than `thinkpoint-0.3.0` and every build stage failed
  on the first `cd`.
- Running `makepkg` from the repository root extracted the release tarball on
  top of the crate's own `src/`, and `makepkg -c` then deleted it. The PKGBUILD
  has moved out of the root so the two cannot collide.
